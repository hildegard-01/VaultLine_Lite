...
## Phase F — 기능 보완 + 보안 강화 ✅ 완료

로컬/서버 공통 기능 보완 및 보안 요소 강화.

### F1 — 태그 기능 전면 개선

- [x] F1-1. TagsPage 완전 재작성 — 좌측 태그 목록 + 우측 파일 검색/자동 규칙 탭
- [x] F1-2. 복수 태그 AND/OR 검색 (`tag:search`)
- [x] F1-3. 자동 태그 규칙 관리 — 확장자/경로/파일명 패턴, 활성 토글
- [x] F1-4. 기존 파일 소급 적용 (`tag:rule:apply-retroactive`)
- [x] F1-5. 커밋 흐름에 `applyAutoTags()` 연결 (CommitService, FileWatcherService)
- [x] F1-6. 사이드바 태그 섹션 제거 (TagsPage로 일원화)

### F2 — 검색·사이드바 보완

- [x] F2-1. 검색에 공유받은 문서 포함 (SearchService + SearchModal "공유" 배지)
- [x] F2-2. 사이드바 공유관리 메뉴 복원 (승인관리 제거)

### F3 — 자동 로그인 + OS 세션 암호화

- [x] F3-1. `SessionService.ts` — `safeStorage` 기반 refresh token 암호화 저장
- [x] F3-2. `ModeManager.initialize()` — 세션 파일 감지 후 자동 커넥티드 전환
- [x] F3-3. `ServerConnectionService.loginWithRefreshToken()` — 토큰 재사용 재인증
- [x] F3-4. 서버 연결/해제 시 세션 자동 저장/삭제
- [x] F3-5. 설정 모달 시스템 탭 — 자동 로그인 기간(1/7/30/90일) + 세션 표시

### F4 — 시스템 통합

- [x] F4-1. Tray 지원 — 창 닫기 시 숨김, 더블클릭 복원, 종료 메뉴
- [x] F4-2. 시작 시 자동 실행 (`app.setLoginItemSettings()`)
- [x] F4-3. `AppSettings`에 `autoLoginDays`, `trayMinimize` 추가
- [x] F4-4. 신규 IPC: `system:startup-get/set`, `session:info/clear`

### F5 — 사용자 계정 관리

- [x] F5-1. 관리자 — `EditUserModal` (표시 이름·이메일·역할 변경)
- [x] F5-2. 일반 사용자 — 내 정보 수정 (`user:update-profile`)
- [x] F5-3. 일반 사용자 — 비밀번호 변경 (`user:change-password`, 8자 이상 검증)
- [x] F5-4. `ServerConnectionService.getMyProfile/updateMyProfile/changePassword()` 신규
